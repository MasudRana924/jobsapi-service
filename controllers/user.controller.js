...
const updateProfile = async (req, res) => {
  try {
    const { firstName, lastName, email, degree, phone, address, education } =
      req.body;
    ...
    const updateUserProfile = await UserModel.updateProfile(
      userId,
      newUserdata
    );
...
const updatePassword = async (req, res, next) => {
  try {
    ...
    const updatedUserPassword = await UserModel.userUpdatePassword(
      userId,
      newPassword
    );
    ...
    // res.created(responseData, "User password is updated");
  } catch (err) {
    // Handle errors
    errorResponseHandler(err, req, res);
  }
